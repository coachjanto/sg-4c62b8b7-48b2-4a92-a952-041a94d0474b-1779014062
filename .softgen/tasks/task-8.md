...
## Checklist
- [ ] Add Channel Production Settings section di Settings page (below Integration Control Center)
- [ ] 3-channel cards dengan production configuration fields per channel
- [ ] Production speed settings: daily target, weekly target, production mode, schedule pattern
- [ ] Content format mix: shorts %, longform %, compilation % (sliders, total must = 100%)
- [ ] Approval mode: manual/auto-publish/schedule-only
- [ ] Resource limits: max budget per provider per week, stop threshold
- [ ] Priority level: low/normal/high/main growth channel
- [ ] Summary panel: total targets all channels, estimated usage, approval load, risk level
- [ ] Risk indicators: low (emerald), medium (amber), high (rose) based on targets vs resources
